...
#
autoria_index = graph_db.get_or_create_index(neo4j.Relationship, "Autoria")
tem_keyword_index= graph_db.get_or_create_index(neo4j.Relationship, "Tem_keyword")
publicado_em_index = graph_db.get_or_create_index(neo4j.Relationship, "Publicacao")
...
for i in range(0, len(pedacos)):
	...
	for j in range(0,  len(pedacos[i][2])):  # o indice corre na lista de nivel 3, de autores
		...
		#autoria = graph_db.get_or_create_relationships((pedacos[i][1], "tem_como_autor", pedacos[i][2][j]))
